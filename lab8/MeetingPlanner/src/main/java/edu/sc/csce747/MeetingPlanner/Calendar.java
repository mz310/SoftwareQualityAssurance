...
public class Calendar {
	...
	public Calendar(){
		...
		occupied.get(11).get(30).add(new Meeting(11,30,"Day does not exist")); // fix boundaries of month based on white box testing
	...
	public static void checkTimes(int mMonth,int mDay,int mStart, int mEnd) throws TimeConflictException{
		...
		if(mMonth < 1 || mMonth >12){ //fix boundary based on white box testing
    ...
    public void addMeeting(Meeting toAdd) throws TimeConflictException {
        ...
        // 1. Огноо, цагийг шалгана
        checkTimes(mMonth, mDay, mStart, mEnd);

        // 2. Тухайн өдрийн бүх meeting-үүдийг авна
        ArrayList<Meeting> thatDay = occupied.get(mMonth).get(mDay);
        boolean booked = false;
        Meeting conflict = null;

        // 2.1 Хэрэв зөвхөн "Day does not exist" marker байвал → энэ өдөр дээр meeting нэмж болохгүй
        if (!thatDay.isEmpty()) {
            boolean onlyMarker = true;
            for (Meeting m : thatDay) {
                if (!"Day does not exist".equals(m.getDescription())) {
                    onlyMarker = false;
                    break;
                }
            }
        ...
        // 3. Бусад жинхэнэ уулзалтуудтай цаг давхцаж байна уу?
        for (Meeting toCheck : thatDay) {
            // marker-уудыг тооцохгүй
            if ("Day does not exist".equals(toCheck.getDescription())) {
                continue;
            }

            // Энгийн overlap шалгалт
            boolean startInside = mStart >= toCheck.getStartTime() && mStart < toCheck.getEndTime();
            boolean endInside = mEnd > toCheck.getStartTime() && mEnd <= toCheck.getEndTime();
            boolean coversExisting = mStart <= toCheck.getStartTime() && mEnd >= toCheck.getEndTime();

            if (startInside || endInside || coversExisting) {
                booked = true;
                conflict = toCheck;
                break;
            }
